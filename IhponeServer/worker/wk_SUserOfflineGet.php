...
class wk_SUserOfflineGet extends e_ServiceWorker{
    function Admin($cookie,$cookieKey){
        ...
        if($roleS && $serverIndex && $setType && $username && $roleSType){
            ...
            $setType_cl=$roleSType.'*'.$setType;
            if($sendStatus){
                $Ctime=time();

                $this->DBinsert('UserControlLog',"serverUNID,roleS,CTime,message,setType,setUser,sendType","'{$result['GameServerUnid']}','{$roleS}','{$Ctime}','{$message}','{$setType_cl}','{$username}',1");
                $this->response->end('{"status":2001,"message":"ok！"}');
            }else{
                $Ctime=time();
                $this->DBinsert('UserControlLog',"serverUNID,roleS,CTime,message,setType,setUser,sendType","'{$result['GameServerUnid']}','{$roleS}','{$Ctime}','{$message}','{$setType_cl}','{$username}',0");
