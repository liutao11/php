    ...
    function Admin($cookie,$cookieKey){
        ...
        if($serverIndex){
            ...
            for($i=0;$i<$dayLength;$i++){
                ...
                //范围内登入用户
                if(date("Y_m_d")==$dayKey){
                    $sql="select PlayerID from {$LOGname}.dbo.LogMsg GROUP BY PlayerID";
                }else{
                    $sql="select PlayerID from {$LOGname}.dbo.ChartLog_{$dayKey} GROUP BY PlayerID";
                }
                $result_aa=$dblinkS->query($sql);
                $userSum[]=$result_aa[$serverIndex];

            }
            $userSumGroup=[];
            foreach($userSum as $item){
                foreach($item as $vv){
                    if(!in_array($vv['PlayerID'],$userSumGroup)){
                        $userSumGroup[]=$vv['PlayerID'];
                    }
                }
            };

            $userSumGroupSum=count($userSumGroup);
            unset($userSumGroup);
        }else{
            $data=[];
        }
        $this->assign('userSumGroupSum',$userSumGroupSum);
        var_dump($userSumGroupSum);
