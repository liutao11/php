...
class wk_NoticeAddPost extends e_OperatorsWorker{
        ...
        if($title && $message) {
            ...
            if($noticeType==1){                          //有发布时间得时候
               ...
               if($startDayTime>$nowTime){
                  if($timeNumber>1){
                      if($timeLong){    //有循环次数
                          $result=$this->DBInsertResultId("NoticeList","objectId,serverIndex,title,message,startDay,timeNumber,timeLong,sendNum,fromId","'{$objectId}','{$serverIndex}','{$title}','{$message}','{$startDayTime}','{$timeNumber}','{$timeLong}','0',{$formId}");
               ...
               if($timeNumber>1){               //需要重复
                   if($timeLong){
                       ...
                       $startDayTime=time();
                       $result=$this->DBInsertResultId("NoticeList","objectId,serverIndex,title,message,startDay,timeNumber,timeLong,sendNum,fromId","'{$objectId}','{$serverIndex}','{$title}','{$message}','{$startDayTime}','{$timeNumber}','{$timeLong}','1',{$formId}");
    ...
    function sendToGameServer($NetUrl,$TelnetPort,$message){
        try {
            ...
            if ($client->connect($NetUrl, $TelnetPort, -1)) {
