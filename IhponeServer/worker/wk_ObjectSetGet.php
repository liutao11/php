...
class wk_ObjectSetGet extends  e_AdminWorker{
    function Admin($cookie,$cookieKey){
        ...
        if($title){
            if($isModify==1){
                $result = $this->DBupdate("objectS","title='{$title}',titleUnid='{$titleUnid}', sendPassword='{$sendPassword}', bottomStr='{$bottomStr}', orgUrl='{$orgUrl}', bbsUrl='{$bbsUrl}', payUrl='{$payUrl}', VIPStrUrl='{$VIPStrUrl}', GMUrl='{$GMUrl}', wallowUrl='{$wallowUrl}', downloadUrl='{$downloadUrl}'","id='{$ObjectId}'");
            }else {
                $times = time();
                $result = $this->DBinsert("objectS", 'title,titleUnid, sendPassword, bottomStr, orgUrl, bbsUrl, payUrl, VIPStrUrl, GMUrl, wallowUrl, downloadUrl,timeS', "'{$title}','{$titleUnid}', '{$sendPassword}','{$bottomStr}','{$orgUrl}','{$bbsUrl}','{$payUrl}','{$VIPStrUrl}','{$GMUrl}','{$wallowUrl}','{$downloadUrl}','{$times}'");
            }
            if($result){
