    ...
    function POST_FUN(){
        ...
        if($name && $passwd){
            ...
            if($result){
                ...
                $this->rd->setex($cookieName.$cookie,3600*2,json_encode(array("joinState"=>1,"unid"=>$result['id'],"username"=>$name,"chinaName"=>$result['chinaName'],"powerJson"=>$result['powerJson'],"groundId"=>$result['groundId'],"objectGround"=>$result['objectGround'],'objectName'=>$objectName,'pageType'=>$pageType,"fromIdmId"=>$result['UFromIdmId'],"fromIdsId"=>$result['UFromIdsId'],'fromIdmName'=>$result['UFromIdmName'],"fromIdsName"=>trim($result["UFromIdsName"]))));
